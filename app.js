...
function trigger(){
  ...
  const duration=Math.max(1500,Math.min(5200,1200+(from-real)/18));const t0=performance.now();
  function frame(t){
...
$("start").onclick = async () => {
  targetStartMs = chosenStartMs();

  state.startTime = String(targetStartMs);
  state.sens = Number($("sens").value);
  saveState();

  showingChosenTime = true;
